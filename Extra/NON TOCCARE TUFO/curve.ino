...
#define RAMP_THRESHOLD    6.0
#define RAMP_SPEED_UP     190
#define RAMP_SPEED_DOWN   70
#define RAMP_KP_FACTOR    0.4
#define RAMP_KD_FACTOR    0.4
...
void moveRobot() {
  ...
  if (currentCommand == "drive") {
    bool onRamp = abs(robotPitch) > RAMP_THRESHOLD;

    if (onRamp) {
      currentSpeed = (robotPitch < 0) ? RAMP_SPEED_UP : RAMP_SPEED_DOWN;
      float correction = computePid(0, currentOffset,
                                    PID_STRAIGHT_KP * RAMP_KP_FACTOR,
                                    PID_STRAIGHT_KI,
                                    PID_STRAIGHT_KD * RAMP_KD_FACTOR);
