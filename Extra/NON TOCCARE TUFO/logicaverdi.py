        ...
        if M["m00"] != 0:
            ...
            if area_rettangolo > MIN_GREEN_RECT_AREA:
                ...
                if bianchi_sop > neri_sop:
                    ...
                    valid_sx = pixel_vvSx > MIN_GREEN_PIXELS_SIDE
                    valid_dx = pixel_vvDx > MIN_GREEN_PIXELS_SIDE

                    if valid_sx and valid_dx:
                        curva = [1, 1]

                    elif valid_sx and not valid_dx:
                        curva = [1, 0]
                        start_cut = (0, end_point1[1] - 50)
                        end_cut   = (end_point1[0] + 100, 480)
                        cv2.rectangle(img_pulita, start_cut, end_cut, (0, 255, 0), 2)

                    elif valid_dx and not valid_sx:
                        curva = [0, 1]
...
def _cur(img_pulita, img_nocrop):
    ...
    pixel_vvSx = cv2.countNonZero(VV_LEFT_RECT)
    pixel_vvDx = cv2.countNonZero(VV_RIGHT_RECT)
    print(f"VV SX: {pixel_vvSx} | VV DX: {pixel_vvDx}")
    print(f"VALID SX: {pixel_vvSx > MIN_GREEN_PIXELS_SIDE} | VALID DX: {pixel_vvDx > MIN_GREEN_PIXELS_SIDE}")
...
def run(cleanImage):
    ...
    if area_sopra > MIN_TOP_LINE_AREA:
        curva_raw = _cur(copia_pulita, img_senza_crop)

        global _intersectionCounter, _lastCurvaDetected

        if curva_raw != [0, 0]:
            if curva_raw == _lastCurvaDetected:
                _intersectionCounter += 1
            else:
                _intersectionCounter = 1
                _lastCurvaDetected = curva_raw
        else:
            _intersectionCounter = 0
            _lastCurvaDetected = [0, 0]

        if _intersectionCounter >= INTERSECTION_CONFIRM_FRAMES:
            curva = _lastCurvaDetected
        else:
            curva = [0, 0]
