...
GREEN_INITIAL_VALUES = [10, 108, 29, 85, 235, 255] #HSV ? lower: [10, 108, 29]  upper: [85, 235, 255]
...
MIN_GREEN_RECT_AREA = 5000
MAX_GREEN_PIXELS_TOTAL = 100000
MIN_GREEN_PIXELS_SIDE = 1000
MIN_LINE_CONTOUR_AREA = 500
MIN_TOP_LINE_AREA = 1000
TURN_180_GREEN_LIMIT = 100000
...
VV_RIGHT_PANEL = (600, 0, 800, 600)
VV_LEFT_PANEL = (0,   0, 200, 600)
...
def on_trackbar_change(value):
    ...
    h_max = cv2.getTrackbarPos("H_max", WINDOW_NAME_GREEN) #HSV ? lower: [10, 108, 29]  upper: [85, 235, 255]
...
cv2.createTrackbar("H_min", WINDOW_NAME_GREEN, GREEN_INITIAL_VALUES[0], 64, on_trackbar_change)
cv2.createTrackbar("S_min", WINDOW_NAME_GREEN, GREEN_INITIAL_VALUES[1], 127, on_trackbar_change)
cv2.createTrackbar("V_min", WINDOW_NAME_GREEN, GREEN_INITIAL_VALUES[2], 55, on_trackbar_change)
cv2.createTrackbar("H_max", WINDOW_NAME_GREEN, GREEN_INITIAL_VALUES[3], 135, on_trackbar_change)
cv2.createTrackbar("S_max", WINDOW_NAME_GREEN, GREEN_INITIAL_VALUES[4], 235, on_trackbar_change)
...
def incr(img_pulita, img_nocrop):
    curva = [0, 0]
  ...
  #  print(f"Sx: {pixel_vvSx}  Dx: {pixel_vvDx}")
        ...
        if M["m00"] != 0:
           ...
           # print(area_rettangolo)
            if area_rettangolo > MIN_GREEN_RECT_AREA:
                ...
                cut_latdx  = copia_nocrop[start_point2[1]:end_point2[1], start_point2[0]:end_point2[0]]
                ...
                if bianchi_sop > neri_sop:
                    ...
                    curva = [0, 0]
            elif pixel_vvSx > MIN_GREEN_PIXELS_SIDE and pixel_vvDx > MIN_GREEN_PIXELS_SIDE:
                curva = [1, 1]
        ...
        if M["m00"] != 0:
            ...
            if area_rettangolo > MIN_GREEN_RECT_AREA:
                ...
                if bianchi_sop > neri_sop:
                    ...
                    if bianchi_latdx > neri_latdx: #sinistra
                        if pixel_vvSx > MIN_GREEN_PIXELS_SIDE and pixel_vvDx > MIN_GREEN_PIXELS_SIDE:
                            curva = [1, 1]
                        else:
                            curva = [1, 0]
                            
                    elif bianchi_latdx < neri_latdx: #destra
...
def main():
    ...
    while True:
        ...
        if area_sopra > MIN_TOP_LINE_AREA:
            curva = cur(copia_pulita, img_senza_crop)
