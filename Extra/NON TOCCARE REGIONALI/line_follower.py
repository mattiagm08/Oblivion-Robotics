...
def run(cleanImage):
    ...
    global lastKnownCenter

    cropX = (800 - 640) // 2
    cropY = (600 - 480) // 2
    cleanImage = cleanImage[cropY:cropY + 480, cropX:cropX + 640]
    ...
    def sendIfChanged(commandName, commFunction):
        global lastCommand
    ...
    if areaTop > config.MIN_TOP_LINE_AREA:
        ...
        if curve[0] == 0 and curve[1] == 0:
            if (config.LEFT_THRESHOLD <= centerTop[0] <= config.RIGHT_THRESHOLD) or (-config.DELTA_X_LIMIT < deltaX < config.DELTA_X_LIMIT):
                print("[DEBUG] Stato: AVANTI")
                comm.forward()
            elif centerTop[0] < config.LEFT_THRESHOLD:
                print("[DEBUG] Stato: CORREZIONE SINISTRA")
                comm.left()
            elif centerTop[0] > config.RIGHT_THRESHOLD:
                print("[DEBUG] Stato: CORREZIONE DESTRA")
                comm.right()
                
        # [INCROCIO SINISTRO]

        elif curve[0] == 1 and curve[1] == 0:
            print("[DEBUG] Stato: INCROCIO A SINISTRA")
            comm.leftIntersection()
            
        # [INCROCIO DESTRO]

        elif curve[0] == 0 and curve[1] == 1:
            print("[DEBUG] Stato: INCROCIO A DESTRA")
            comm.rightIntersection()
            
        # [INVERSIONE]

        elif curve[0] == 1 and curve[1] == 1:
            print("[DEBUG] Stato: INVERSIONE 180")
            comm.turn180()
    else:

        # LOGICA PER QUANDO MANCA LA LINEA SUPERIORE

        if lastKnownCenter < config.LEFT_THRESHOLD:
            print("[DEBUG] Stato: RECUPERO LINEA (SINISTRA)")
            sendIfChanged("RECUPERO_SX", comm.left)
        elif lastKnownCenter > config.RIGHT_THRESHOLD:
            print("[DEBUG] Stato: RECUPERO LINEA (DESTRA)")
            sendIfChanged("RECUPERO_DX", comm.right)
