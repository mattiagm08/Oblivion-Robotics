  ...
  void muoviRobot() {
    ...
    if (currentShake) {
      // ── SHAKE: retromarcia sterzante lenta ─────────────────────
      // currentSpeed è negativo. Correzione ridotta al 40% per evitare
      // over-steer in retromarcia. Nessun tight-curve.
      speedL = TURN_SPEED;
      speedR = -TURN_SPEED;
    }
    else {
      // ── NORMALE: logica originale invariata ────────────────────
      if (correzione > 12) {
        speedL = TURN_SPEED + abs(correzione / 2);
        speedR = -speedL * 0.85;
      }
      else if (correzione < -12) {
        speedR = TURN_SPEED + abs(correzione / 2);
        speedL = -speedR * 0.85;
      }
      else {
  ...
  void leggiSeriale() {
    while (Serial.available()) {
      ...
      if (c == '<') { serialBuffer = ""; isReceiving = true; }
        ...
        int shIdx  = serialBuffer.indexOf("SH:");   // ← nuovo

        if (offIdx != -1 && spdIdx != -1) {
          int commaOff = serialBuffer.indexOf(',', offIdx);
          int commaSpd = serialBuffer.indexOf(',', spdIdx);

          currentOffset = serialBuffer.substring(offIdx + 4, commaOff).toFloat();

          // SPD termina con ',' se SH è presente, altrimenti fine stringa
          String spdStr = (commaSpd != -1)
                          ? serialBuffer.substring(spdIdx + 4, commaSpd)
                          : serialBuffer.substring(spdIdx + 4);
          currentSpeed = spdStr.toInt();
        }

        // SH opzionale: se assente mantiene il valore precedente
        if (shIdx != -1) {
          currentShake = (serialBuffer.substring(shIdx + 3).toInt() == 1);
        }
      }
